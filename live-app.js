(()=>{
  ...
  const toggle=$('liveToggle'),toggleIcon=$('liveToggleIcon'),toggleLabel=$('liveToggleLabel'),prev=$('livePrev'),next=$('liveNext'),queue=$('liveQueue'),liveReset=$('liveReset'),spotifyState=$('spotifyPlayerState'),spotifyCard=$('spotifyPlayerCard');
  ...
  let currentProgram=null,ride=[],index=0,offset=0,elapsed=Math.max(0,Number(saved.elapsed)||0),running=false,runningBaseElapsed=elapsed,runningStartedAt=0,lastSecond=-1,audioCtx,wakeLock,spotifyController=null,spotifyLoadedUri='',spotifyPreviewComplete=false,returnFocus=null;
  ...
  function selectProgram(id){
    ...
    buildQueue();render();requestAnimationFrame(()=>toggle.focus());
  }

  const trackUri=track=>{const url=track?.embedUrl;if(!url||!url.includes('/track/'))return null;return `spotify:track:${url.split('/track/')[1].split(/[?#]/)[0]}`};
  const trackIndexFromUri=uri=>ride.findIndex(track=>trackUri(track)===uri);
  function excludeSpotifyFrameFromTabOrder(){const frame=spotifyCard.querySelector('iframe');if(frame)frame.tabIndex=-1}
  new MutationObserver(excludeSpotifyFrameFromTabOrder).observe($('spotifyEmbed'),{childList:true,subtree:true});
  function loadSpotifyForCurrent(){
    if(!ride.length)return;const uri=trackUri(ride[index]);
    if(!uri){spotifyCard.classList.add('external-only');spotifyCard.classList.remove('embed-ready');spotifyState.textContent='Use Find in Spotify for this track';return}
    spotifyCard.classList.remove('external-only');
    if(!spotifyController){spotifyState.textContent='Loading preview…';return}
    const changed=uri!==spotifyLoadedUri;
    if(changed){spotifyLoadedUri=uri;spotifyPreviewComplete=false;spotifyState.textContent='Loading preview…';spotifyController.loadEntity(uri)}
  }
  function handleSpotifyPlayback(event){
    const data=event&&event.data?event.data:event;if(!data)return;
    const duration=Math.max(0,Number(data.duration)||0)/1000,position=Math.max(0,Number(data.position)||0)/1000,isPreview=duration>0&&duration<60,ended=duration>0&&position>=duration-.75;
    if(isPreview&&ended)spotifyPreviewComplete=true;
    const timing=duration?`${fmt(position)} / ${fmt(duration)}`:fmt(position);
    spotifyState.textContent=data.isBuffering?'Buffering…':isPreview&&ended?'Preview complete · timer continues':data.isPaused?`Paused · ${timing}`:isPreview?`Preview playing · ${timing}`:`Playing · ${timing}`;
  }
  window.onSpotifyIframeApiReady=IFrameAPI=>{
    const fallback=programs[0]?.tracks[0];
    IFrameAPI.createController($('spotifyEmbed'),{uri:trackUri(fallback),width:'100%',height:152},controller=>{
      spotifyController=controller;spotifyLoadedUri=trackUri(fallback)||'';
      controller.addListener('ready',()=>{spotifyCard.classList.add('embed-ready');excludeSpotifyFrameFromTabOrder();if(currentProgram)loadSpotifyForCurrent(false)});
      controller.addListener('playback_started',()=>{spotifyState.textContent='Starting playback…'});
      controller.addListener('playback_update',handleSpotifyPlayback);
    });
  };
  setTimeout(()=>{if(!spotifyController&&currentProgram)spotifyState.textContent='Player unavailable · use Spotify link'},9000);
  ...
  function render(){
    ...
    $('elapsedTime').textContent=fmt(sessionElapsed);$('remainingTime').textContent=fmt(Math.ceil(remaining));$('liveBar').style.width=`${Math.min(100,(offset/track.seconds)*100)}%`;$('liveSpotify').href=track.url;
    const spotifyLabel=$('liveSpotify').querySelector('span');if(spotifyLabel)spotifyLabel.textContent=track.embedUrl?'Open in Spotify':'Find in Spotify';
    $('liveSpotify').setAttribute('aria-label',track.embedUrl?'Open current song in Spotify':'Find current song in Spotify');
    ...
    toggleIcon.textContent=running?'Ⅱ':'▶';toggleLabel.textContent=running?'Pause':'Start timer';toggle.setAttribute('aria-label',running?'Pause timer':'Start timer');prev.disabled=index===0&&offset<1;next.textContent=index===ride.length-1?'Finish ✓':'Next →';
  ...
  function setRunning(value){
    ...
    reconcileClock(false);
    if(value&&elapsed>=totalSeconds()){elapsed=0;index=0;offset=0}
    running=value;runningBaseElapsed=elapsed;runningStartedAt=running?Date.now():0;
    if(running)requestWakeLock();else releaseWakeLock();
    save();render();
  }
  function setElapsed(value){elapsed=Math.max(0,Math.min(value,totalSeconds()));runningBaseElapsed=elapsed;if(running)runningStartedAt=Date.now();const located=locate(elapsed);index=located.index;offset=located.offset;lastSecond=-1;save();render()}
  function jump(i){setElapsed(before(Math.min(Math.max(i,0),ride.length-1)));loadSpotifyForCurrent()}
  function advance(auto=false){if(index<ride.length-1){setElapsed(before(index+1));if(auto)transitionSignal();loadSpotifyForCurrent()}else{setElapsed(totalSeconds());setRunning(false);transitionSignal()}}
  function tick(){
    if(running){const prior=index;reconcileClock(false);const remaining=ride[index].seconds-offset,second=Math.ceil(remaining);if(second!==lastSecond){lastSecond=second;if(second===10)tone(520,.1);if(second>0&&second<=3)tone(620+(3-second)*90,.08);save()}if(index!==prior){transitionSignal();loadSpotifyForCurrent()}render()}
  ...
  overlay.addEventListener('keydown',event=>{if(event.key==='Escape'){closeLiveMode();return}if(event.key!=='Tab')return;const focusable=[...overlay.querySelectorAll('button:not([disabled]),a[href],[tabindex]:not([tabindex="-1"])')].filter(element=>element.getClientRects().length);if(!focusable.length)return;const first=focusable[0],last=focusable[focusable.length-1];if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}});
  ...
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible'&&running){reconcileClock(true);render();requestWakeLock()}});
  addEventListener('pageshow',()=>{if(running){reconcileClock(true);render()}});
  addEventListener('focus',()=>{if(running){reconcileClock(true);render()}});
  function resetLiveSession(){if(!currentProgram)return;setRunning(false);elapsed=0;runningBaseElapsed=0;runningStartedAt=0;index=0;offset=0;lastSecond=-1;save();render();queue.scrollTo({top:0,behavior:'smooth'})}
